...
ax.plot(x_fit, y_fit, linestyle='-', label=f"fit: y = {slope}({std_err_slope})$\\,$(Unit) {pt.check_for_positive_intercept(intercept)} {str(intercept).replace("-", "")}({std_err_intercept})$\\,$(Unit)")

ax.legend(loc='upper left', fancybox=False, edgecolor='black')
ax.set_xlabel('X-Axis $Size\\,$(Unit)')
ax.set_ylabel('Y-Axis $Size\\,$(Unit)')
ax.set_title('Title')
